...
function EditProfileView() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) { newErrors.name = "Stage Name is required"; missing.push("Stage Name"); }
    if (!formData.location.trim()) { newErrors.location = "Base Location is required"; missing.push("Base Location"); }
    if (!formData.bio.trim() || formData.bio.trim().length < 20) { newErrors.bio = "Biography must be at least 20 characters"; missing.push("Biography (min 20 chars)"); }
    if (!formData.category) { newErrors.category = "Category is required"; missing.push("Category"); }
    if (!formData.artistType) { newErrors.artistType = "Act Type is required"; missing.push("Act Type"); }
    if (!formData.price || formData.price <= 0) { newErrors.price = "Hourly Rate must be greater than 0"; missing.push("Hourly Rate"); }
    if (formData.experience < 0) { newErrors.experience = "Experience cannot be negative"; missing.push("Experience"); }
    if (formData.genres.length === 0) { newErrors.genres = "At least one Genre is required"; missing.push("Genres"); }
    ...
    <div className="min-h-screen bg-gray-950 text-white pb-20 selection:bg-violet-500/30 pt-8">
      ...
      <div className="max-w-[75rem] mx-auto px-4 lg:px-8">
        ...
        <div className="flex items-center gap-4 mb-10">
            <Link href="/home/artist" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors border border-white/10">
          ...
          {formError && (
            <div className="mb-8 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-start gap-3 text-red-300">
              <div className="mt-0.5">
                <X className="w-5 h-5 text-red-400" />
              </div>
              <div>
                <h3 className="font-bold text-red-400 mb-1">{formError}</h3>
                 ...
                 <div>
                   ...
                   <div className="relative w-full h-40 bg-white/5 rounded-2xl border-2 border-dashed border-white/10 overflow-hidden group">
                       {(coverImagePreview || formData.coverImage) && (
                         <img src={coverImagePreview || formData.coverImage} alt="Cover" className="w-full h-full object-cover opacity-60 group-hover:opacity-30 transition-opacity" />
                     )}
                     <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                       ...
                       <input type="file" disabled={uploadingImage === "cover"} onChange={(e) => handleImageSelect(e, "cover")} accept="image/*" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                   </div>
                 </div>
                   ...
                   <div className="flex items-center gap-6">
                     <div className="relative w-24 h-24 rounded-full bg-white/5 border-2 border-white/10 overflow-hidden shrink-0 group">
                         {(profileImagePreview || formData.profileImage) ? (
                           <img src={profileImagePreview || formData.profileImage} alt="Avatar" className="w-full h-full object-cover opacity-80 group-hover:opacity-40 transition-opacity" />
                       ) : (
                       ...
                       <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                         ...
                         <input type="file" disabled={uploadingImage === "profile"} onChange={(e) => handleImageSelect(e, "profile")} accept="image/*" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                     </div>
             ...
             <section className="bg-[#1E112A]/40 border border-white/10 backdrop-blur-xl rounded-3xl p-6 md:p-8 space-y-6">
               ...
               <div className="grid sm:grid-cols-2 gap-6">
                 <div>
                   ...
                   <input type="text" value={formData.name} onChange={e => { setFormData({ ...formData, name: e.target.value }); if (errors.name) setErrors(prev => ({ ...prev, name: "" })); }} className={`w-full bg-white/5 border ${errors.name ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500' : 'border-white/10 focus:border-violet-500 focus:ring-violet-500'} rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-1 transition-all font-medium`} placeholder="E.g. The Weeknd" />
                   {errors.name && <p className="text-red-400 text-[10px] mt-1.5 uppercase font-bold tracking-wider">{errors.name}</p>}
                 </div>
                 <div>
                   ...
                   <input type="text" value={formData.location} onChange={e => { setFormData({ ...formData, location: e.target.value }); if (errors.location) setErrors(prev => ({ ...prev, location: "" })); }} className={`w-full bg-white/5 border ${errors.location ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500' : 'border-white/10 focus:border-violet-500 focus:ring-violet-500'} rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-1 transition-all font-medium`} placeholder="E.g. London, UK" />
                   {errors.location && <p className="text-red-400 text-[10px] mt-1.5 uppercase font-bold tracking-wider">{errors.location}</p>}
                 </div>
               </div>

               <div>
                 <label className="text-sm font-bold text-gray-400 uppercase tracking-widest block mb-2">Biography</label>
                 <textarea rows={5} value={formData.bio} onChange={e => setFormData({ ...formData, bio: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all font-medium resize-none custom-scrollbar" placeholder="Tell your story. What makes your performances unique?" />
               </div>
               ...
               <div className="grid sm:grid-cols-2 gap-6">
                 <div>
                   ...
                   <select value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} className="w-full bg-gray-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all font-medium">
                 ...
                 <div>
                   ...
                   <select value={formData.artistType} onChange={e => setFormData({ ...formData, artistType: e.target.value })} className="w-full bg-gray-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-all font-medium">
               ...
               <div className="space-y-5">
                 <div>
                   ...
                   <div className="relative">
                     ...
                     <input type="number" min="0" value={formData.price} onChange={e => setFormData({ ...formData, price: Number(e.target.value) })} className="w-full bg-black/40 border border-white/5 rounded-xl pl-9 pr-4 py-3 text-white focus:outline-none focus:border-amber-500/50 transition-all font-black text-xl" />
                   </div>
                 </div>
                 ...
                 <div>
                   ...
                   <input type="number" min="0" value={formData.experience} onChange={e => setFormData({ ...formData, experience: Number(e.target.value) })} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500/50 transition-all font-bold" />
                 </div>
               </div>
             </section>

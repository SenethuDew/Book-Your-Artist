...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float sigma = max(0.001, 0.18 * widthScale * uScanSoftness); float sigmaA = sigma * 2.0;
    float combinedPulse = 0.0; float combinedAura = 0.0;
    float cycle = dur + del; float tCycle = mod(iTime, cycle);
    float scanPhase = clamp((tCycle - del) / dur, 0.0, 1.0); float phase = scanPhase;
    ...
    float headFade = smoother01(0.0, headW, phase); float tailFade = 1.0 - smoother01(1.0 - tailW, 1.0, phase);
    ...
    for (int i = 0; i < MAX_SCANS; i++) {
        ...
        float tActiveI = iTime - uScanStarts[i]; float phaseI = clamp(tActiveI / dur, 0.0, 1.0);
        ...
        float headFadeI = smoother01(0.0, headW, phaseI); float tailFadeI = 1.0 - smoother01(1.0 - tailW, 1.0, phaseI);
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full overflow-hidden ${className ?? ''}`} style={style}>
      {showPreview && (
        <div className="right-3 bottom-3 absolute bg-black shadow-[0_4px_16px_rgba(0,0,0,0.4)] border border-white/25 rounded-lg w-[220px] h-[132px] overflow-hidden">
          <video ref={videoRef} muted playsInline autoPlay className="w-full h-full object-cover -scale-x-100" />
          <div className="top-2 left-2 absolute bg-black/50 backdrop-blur-[4px] px-[6px] py-[2px] rounded-[6px] text-white text-xs">
